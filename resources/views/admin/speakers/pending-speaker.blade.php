    ...
    <div class="">
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
            <div class="flex items-center gap-3">
                <div>
                    <h1 class="text-2xl font-bold text-primary font-montserrat">Pending Speakers</h1>
        ...
        <div class="bg-white shadow rounded-xl border border-primary-100 overflow-hidden">
            ...
            <div
                class="px-6 py-4 border-b border-primary-100 bg-primary-50 flex flex-wrap items-center justify-between gap-4">
                <div class="flex items-center gap-4">
                    <span class="text-sm font-medium text-primary-700 font-lato">
                        Showing <span
                            class="font-semibold font-montserrat">{{ $speakers->firstItem() }}–{{ $speakers->lastItem() }}</span>
                        of <span class="font-semibold font-montserrat">{{ $speakers->total() }}</span> speakers
                    </span>
                    @if (request()->has('search'))
                        <span
                            class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-primary/10 text-primary font-montserrat">
                            Filtered
                            <button type="button"
                ...
                <div class="flex items-center gap-2">
                    <label for="sort" class="text-sm font-medium text-primary-700 whitespace-nowrap font-lato">Sort
                        by:</label>
                    <select id="sort"
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-primary-100">
                    <thead class="bg-primary-50">
                        <tr>
                            <th scope="col"
                                class="px-6 py-3 text-left text-xs font-semibold text-primary uppercase tracking-wider font-montserrat">
                                Speaker</th>
                            <th scope="col"
                                class="px-6 py-3 text-left text-xs font-semibold text-primary uppercase tracking-wider font-montserrat">
                            ...
                            <th scope="col"
                                ...
                                Status</th>
                            <th scope="col"
                                class="px-6 py-3 text-right text-xs font-semibold text-primary uppercase tracking-wider font-montserrat">
                                Actions</th>
                        </tr>
                    </thead>
                    <tbody class="bg-white divide-y divide-primary-100">
                        @forelse ($speakers as $speaker)
                            <tr class="hover:bg-primary-50 transition-colors">
                                    ...
                                    <div class="flex items-center gap-3">
                                        <div class="flex-shrink-0 h-10 w-10 relative">
                                            <img class="h-10 w-10 rounded-full object-cover border border-primary-100"
                                                src="{{ $speaker->user->photo ? asset('storage/' . $speaker->user->photo) : 'https://i.pravatar.cc/40?u=' . $speaker->email }}"
                                                alt="{{ $speaker->user->name }}">
                                            @if ($speaker->is_featured)
                                                <span
                                                    class="absolute -top-1 -right-1 bg-accent text-white rounded-full p-0.5">
                                                    <i data-lucide="star" class="w-3 h-3 fill-current"></i>
                                                </span>
                                            @endif
                                        </div>
                                        <div>
                                            <div class="font-medium text-primary font-montserrat">
                                                {{ $speaker->user->name }}</div>
                                ...
                                <td class="px-6 py-4">
                                    @if ($speaker->user->headline || $speaker->organization)
                                        <div class="text-sm font-medium text-primary font-montserrat">
                                            {{ $speaker->user->headline ?? '—' }}</div>
                                        <div class="text-sm text-gray-500 font-lato">{{ $speaker->organization ?? '—' }}
                                        </div>
                                    @else
                                ...
                                <td class="px-6 py-4">
                                    @if ($speaker->status == 'active')
                                        <span
                                            class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-accent-100 text-accent-800 font-montserrat">
                                            Active
                                        </span>
                                    @else
                                        <span
                                            class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-secondary-100 text-secondary-800 font-montserrat">
                                            Inactive
                                        </span>
                                    @endif
                                </td>
                                ...
                                <td class="px-6 py-4 text-right">
                                    <div class="inline-flex gap-2 items-center">
                                        ...
                                        <a href="{{ route('admin.speakers.edit', $speaker) }}" title="Edit"
                                            class="p-2 text-primary hover:text-primary-600 rounded-full hover:bg-primary-50 transition-colors"
                                            aria-label="Edit speaker">
                                            <i data-lucide="edit-3" class="w-5 h-5"></i>
                                        </a>
                                        <div x-data="{ processing: false }" class="inline-flex">
                                            @if (!$speaker->isActive())
                                                <a href="{{ route('admin.speakers.activate', $speaker) }}" title="Approve Speaker"
                                                    class="p-2 text-accent hover:text-accent-600 rounded-full hover:bg-accent-50 transition-colors"
                                                    aria-label="Approve speaker"
                                                    x-bind:class="processing && 'opacity-50 pointer-events-none'"
                                                    x-on:click="processing = true">
                                                    <i data-lucide="check-circle" class="w-5 h-5"></i>
                                                </a>
                                            @else
                                                <a href="" title="Ban Speaker"
                                                    class="p-2 text-secondary hover:text-secondary-600 rounded-full hover:bg-secondary-50 transition-colors"
                                                    aria-label="Ban speaker"
                                                    x-bind:class="processing && 'opacity-50 pointer-events-none'"
                                                    x-on:click="processing = true">
                                        ...
                                        <button data-modal-target="delete-speaker-modal"
                            ...
                            <tr>
                                <td colspan="5" class="px-6 py-16 text-center">
                                    <div class="flex flex-col items-center justify-center space-y-4 text-gray-400">
                                        ...
                                        <h3 class="text-lg font-medium text-primary font-montserrat">No speakers found
                                        </h3>
                                        <p class="max-w-md text-center font-lato">Get started by adding your first
                                            speaker to the event.</p>

                    ...
                    <div class="border border-gray-200 rounded-xl p-6 bg-gray-50">
                        ...
                        <div class="space-y-5">
                            <div>
                                ...
                                <p class="text-gray-900 text-base font-medium">{{ $application->topic_title ?? 'N/A' }}</p>
                            </div>

                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Description</label>
                                <div class="prose prose-sm max-w-none text-gray-800 bg-white p-3 rounded-lg border">
                                    {!! nl2br(e($application->topic_description ?? '<span class="text-gray-400 italic">No description provided.</span>')) !!}
                                </div>
                            </div>
                    ...
                    <div class="border border-gray-200 rounded-xl p-6 bg-gray-50">
                        ...
                        <div class="flex flex-col sm:flex-row gap-6">
                            <div class="flex-shrink-0">
                                ...
                                @else
                                    <div class="h-20 w-20 rounded-full bg-gray-200 flex items-center justify-center border-2 border-dashed border-gray-300">
                                        <span class="text-lg font-bold text-gray-500">
                                            {{ strtoupper(substr($application->speaker->name ?? 'S', 0, 1)) }}
                                        </span>
                                    </div>
                                @endif
                            </div>
                            <div class="flex-grow">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                                    <div class="flex items-center text-sm text-gray-600 p-2 bg-white rounded-lg border">
                                        ...
                                        <span>{{ $application->speaker->email ?? 'No email provided' }}</span>
                                    </div>
                                    <div class="flex items-center text-sm text-gray-600 p-2 bg-white rounded-lg border">
                                        <i data-lucide="phone" class="w-4 h-4 mr-2 text-[#00275E]"></i>
                                        <span>{{ $application->speaker->phone ?? 'No phone provided' }}</span>
                                    </div>
                                </div>
                                ...
                                <div class="mb-4">
                                    ...
                                    <div class="prose prose-sm max-w-none text-gray-800 bg-white p-3 rounded-lg border">
                                        {!! nl2br(e($application->speaker->bio ?? '<span class="text-gray-400 italic">No bio provided.</span>')) !!}
                                    </div>
                                </div>
                        ...
                        <div class="space-y-3">
                            @if($application->status == 'pending')
                                ...
                                <button type="button"
                                    ...
                                    data-method="POST"
                                    data-title="Approve Speaker Application"
                                    ...
                                    class="w-full inline-flex items-center justify-center px-4 py-2.5 border border-transparent text-sm font-medium rounded-lg shadow-sm text-white bg-[#00275E] hover:bg-[#FF0000] focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[#00275E] transition">
                                ...
                                <button type="button"
                                    ...
                                    data-message="Please provide a reason for rejecting {{ $application->speaker->name }}’s application:"
                            ...
                            @elseif($application->status == 'approved')
                                ...
                                <button type="button"
                                    data-modal-target="feedback-modal"
                                    data-modal-toggle="feedback-modal"
                                    data-action="{{ route('admin.speakers.application.revoke', $application) }}"
                                    data-method="POST"
                                    data-spoofMethod="DELETE"
                    ...
                    <div class="border border-gray-200 rounded-xl p-6 bg-gray-50">
                        ...
                        <div class="space-y-3 text-sm">
                            <div class="flex justify-between">
                                ...
                                <span class="font-medium text-gray-900">{{ $application->created_at->format('M d, Y \a\t g:i A') }}</span>
                            </div>
                            @if($application->approved_at)
                                <div class="flex justify-between">
                                    <span class="text-gray-600">Approved</span>
                                    <span class="font-medium text-green-700">{{ $application->approved_at->format('M d, Y \a\t g:i A') }}</span>
                                </div>
                            @endif
                            @if($application->rejected_at)
                                <div class="flex justify-between">
                                    <span class="text-gray-600">Rejected</span>
                                    <span class="font-medium text-red-700">{{ $application->rejected_at->format('M d, Y \a\t g:i A') }}</span>
                                </div>
                            @endif
                            @if($application->updated_at && $application->updated_at != $application->created_at)
                                <div class="flex justify-between">
                                    <span class="text-gray-600">Last Updated</span>
                                    <span class="font-medium text-gray-900">{{ $application->updated_at->format('M d, Y \a\t g:i A') }}</span>
                                </div>
                            @endif
                        </div>
                    </div>

        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
            <div class="flex items-center gap-3">
                <div class="p-2.5 rounded-lg bg-[#00275E]/10">
                    <i data-lucide="calendar-plus" class="w-6 h-6 text-[#00275E]"></i>
                </div>
                <div>
                    <h1 class="text-2xl font-extrabold text-[#00275E]">Create New Event</h1>
        ...
        <div class="bg-white border border-gray-100 rounded-xl shadow-sm p-6 md:p-8">
            <form action="{{ route('admin.events.store') }}" method="POST" enctype="multipart/form-data"
                ...
                <div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                        <x-input name="start_date" label="Start Date & Time" type="datetime-local" required icon="calendar" :value="old('start_date')" />
                        <x-input name="end_date" label="End Date & Time" type="datetime-local" required icon="calendar-check" :value="old('end_date')" />
                        <x-input name="location" label="Location (URL or Venue Name)" required icon="map-pin" :value="old('location')" placeholder="e.g. Zoom Link or Conference Hall" />
                        <x-input name="physical_address" label="Full Physical Address (if offline/hybrid)" icon="map" :value="old('physical_address')" placeholder="Street, City, State, Country" />
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                        <x-input name="contact_email" label="Contact Email" type="email" icon="mail" :value="old('contact_email')" placeholder="organizer@event.com" />
                        <x-input name="entry_fee" label="Entry Fee (₦)" type="number" min="0" step="0.01" icon="credit-card" :value="old('entry_fee', 0)" placeholder="0.00" />
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div>
                        <label for="program_cover" class="block text-sm font-medium text-gray-700 mb-2">Upload Cover Image (JPG/PNG)</label>
                ...
                <div>
                    ...
                    <div class="flex flex-wrap items-center gap-6 p-4 bg-gray-50 rounded-lg">
                        <label class="inline-flex items-center">
                            <input type="checkbox" name="is_active" class="rounded border-gray-300 text-[#00275E] focus:ring-[#00275E]" {{ old('is_active', true) ? 'checked' : '' }}>
                            <span class="ml-2 text-sm font-medium text-gray-700">Active</span>
                        </label>

                        <label class="inline-flex items-center">
                            <input type="checkbox" name="is_published" class="rounded border-gray-300 text-[#00275E] focus:ring-[#00275E]" {{ old('is_published') ? 'checked' : '' }}>
                            <span class="ml-2 text-sm font-medium text-gray-700">Published</span>
                        </label>

                        <label class="inline-flex items-center">
                            <input type="checkbox" name="is_allowing_application" class="rounded border-gray-300 text-[#00275E] focus:ring-[#00275E]" {{ old('is_allowing_application', true) ? 'checked' : '' }}>

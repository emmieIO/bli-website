    ...
    <div class="max-w-5xl mx-auto">
        ...
        <form action="{{ route('admin.events.update', $event) }}" method="POST" enctype="multipart/form-data"
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                ...
                <!-- Cover Image -->
                <x-input name="program_cover" label="Cover Image" type="file" icon="image" accept="image/*" />
                @if ($event->program_cover)
                    <p class="text-sm text-gray-500">
                        Current file:
                        <a href="{{ asset('storage/' . $event->program_cover) }}" class="underline text-[#00275E]" target="_blank">
                            View cover image
                        </a>
                    </p>
                @endif
            ...
            <div class="flex flex-wrap gap-4">
                <label class="inline-flex items-center">
                    <input type="checkbox" name="is_active" class="rounded text-[#00275E]"
                           {{ old('is_active', $event->is_active) ? 'checked' : '' }}>
                    <span class="ml-2 text-sm text-gray-700">Active</span>
                </label>

                <label class="inline-flex items-center">
                    <input type="checkbox" name="is_published" class="rounded text-[#00275E]"
                           {{ old('is_published', $event->is_published) ? 'checked' : '' }}>
                    <span class="ml-2 text-sm text-gray-700">Published</span>
                </label>

                <label class="inline-flex items-center">
                    <input type="checkbox" name="is_allowing_application" class="rounded text-[#00275E]"

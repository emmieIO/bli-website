...
<div class="space-y-5">
    <div class="bg-white border border-primary-100 rounded-xl shadow-sm p-6" data-aos="fade-up" data-aos-duration="600">
        ...
        <form action="{{ route('profile.update') }}" method="POST" class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div class="relative">
                <x-input label="Email" type="email" name="email" value="{{ old('email', $user->email) }}"
                    icon="mail" readonly="{{ $user->hasRole(['student', 'user', 'instructor']) ? 'readonly' : '' }}"
                    required="true" />
            </div>
            <div class="relative">
            ...
            <div class="relative">
                <x-input label="Linkedin Profile" type="text" name="linkedin" icon="linkedin" value="{{ old('linkedin', $user->linkedin) }}" />
            </div>
            <div class="relative">
                <x-input label="Website" type="text" name="linkedin" icon="globe" value="{{ old('website', $user->website) }}" />
            </div>

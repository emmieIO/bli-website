...
<div class="space-y-1" id="{{ $name }}">
    ...
    <!-- Input Container -->
    <div class="flex items-center p-2 rounded-md transition-all duration-150 border border-gray-300 hover:border-orange-600 focus-within:ring-2 focus-within:ring-orange-600/30 focus-within:border-orange-600 {{ $disabled ? 'bg-gray-100 opacity-75 cursor-not-allowed' : 'bg-white' }}"
        ...
        <input
            ...
            {{ $attributes->merge([
                'class' =>
                    'block w-full pl-' .
                    ($icon ? '10' : '3') .
                    'text-gray-900 font-medium rounded-md border-none focus:ring-0 sm:text-sm placeholder-gray-400',
            ]) }} />

        <!-- Password Toggle Button -->
        @if ($type === 'password')
            <button type="button" tabindex="-1"
                class="flex items-center text-gray-500 hover:text-orange-600 focus:outline-none"
                @click="show = !show">

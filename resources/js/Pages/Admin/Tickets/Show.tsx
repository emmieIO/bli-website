          ...
          <div className="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            <div className="p-6 bg-white border-b border-slate-200">
              <div className="flex justify-between items-center mb-4">
                <div>
                  <div className="text-sm text-slate-500 mb-1">Ticket #{ticket.reference_code}</div>
                  <h1 className="text-2xl font-bold">{ticket.subject}</h1>
                </div>
                <Link href={route('admin.tickets.index')} className="inline-flex items-center px-4 py-2 bg-slate-200 text-slate-700 rounded-md">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                ...
                <div>
                  <form onSubmit={submitStatus}>
                    <label htmlFor="status" className="block text-sm font-medium text-slate-700">Change Status</label>
                    <div className="mt-1 flex rounded-md shadow-sm">
                      <select
                        id="status"
                        value={statusData.status}
                        onChange={e => setStatusData('status', e.target.value)}
                        className="block w-full rounded-none rounded-l-md border-slate-300 focus:border-primary-500 focus:ring-primary-500"
                      ...
                      <button type="submit" className="inline-flex items-center px-4 py-2 border border-l-0 border-slate-300 rounded-r-md bg-slate-50 text-sm font-medium text-slate-700 hover:bg-slate-100" disabled={processingStatus || ticket.status === 'closed'}>
              ...
              <div className="space-y-4">
                {ticket.replies.map(reply => (
                  <div key={reply.id} className="p-4 rounded-lg bg-slate-100">
                    <div className="flex justify-between items-center mb-2">
                      <p className="font-bold">{reply.user.name}</p>
                      <p className="text-sm text-slate-500">{new Date(reply.created_at).toLocaleString()}</p>
                    </div>
                    <p>{reply.message}</p>
                  </div>
                ))}
              ...
              <div className="mt-8">
                <form onSubmit={submitReply}>
                  <div className="mb-4">
                    <label htmlFor="message" className="block text-sm font-medium text-slate-700">Your Reply</label>
                    <textarea
                      id="message"
                      rows={4}

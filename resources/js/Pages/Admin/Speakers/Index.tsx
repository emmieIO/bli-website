...
export default function SpeakersIndex({ speakers }: SpeakersProps) {
    ...
    <DashboardLayout sideLinks={sideLinks}>
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
        <div className="flex items-center gap-3">
          <div>
            <h1 className="text-2xl font-bold text-primary">Active Speakers</h1>
        ...
        <Link
          ...
          className="inline-flex items-center px-4 py-2 bg-primary text-white text-sm font-medium rounded-lg hover:bg-primary-600 focus:ring-4 focus:ring-primary-300 transition shadow-sm"
        >
        ...
        <div className="px-6 py-4 border-b border-slate-200 bg-slate-50 flex flex-wrap items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            <span className="text-sm font-medium text-slate-700">
              Showing <span className="font-semibold">{speakers.from}–{speakers.to}</span>{' '}
              of <span className="font-semibold">{speakers.total}</span> speakers
            </span>
          </div>
          <div className="flex items-center gap-2">
            ...
            <select
              ...
              className="block w-full pl-3 pr-10 py-2 text-sm border-slate-300 focus:outline-none focus:ring-2 focus:ring-primary focus:border-primary rounded-md"
            >
                    ...
                    <td className="px-6 py-4 text-right">
                      <div className="inline-flex gap-2">
                        ...
                        <Link
                          ...
                          title="Edit"
                          className="p-1.5 text-primary hover:text-primary-600 rounded-full hover:bg-primary-50 transition-colors"
                        >
                          <i className="fas fa-edit w-5 h-5"></i>
                        </Link>
                        {speaker.application_status === 'pending' && (
                          <button
                            title="Review Application"
                            className="p-1.5 text-accent hover:text-accent-600 rounded-full hover:bg-accent-50 transition-colors"
                          >
                            <i className="fas fa-clipboard-check w-5 h-5"></i>
                          </button>
                        )}
                        <button
                          onClick={() => openDeleteModal(speaker)}
                          title="Delete"
                          className="p-1.5 text-red-600 hover:text-red-700 rounded-full hover:bg-red-50 transition-colors"
                        >
                  ...
                  <td colSpan={4} className="px-6 py-16 text-center">
                    <div className="flex flex-col items-center justify-center space-y-4 text-gray-400">
                      ...
                      <div className="flex flex-col sm:flex-row gap-3 mt-4">
                        <Link
                          href={route('admin.speakers.applications.pending')}
                          className="inline-flex items-center px-4 py-2 bg-white border border-slate-300 text-slate-700 text-sm font-medium rounded-lg hover:bg-slate-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary transition shadow-sm"
                        >
      ...
      {showDeleteModal && selectedSpeaker && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-sm">
          <div className="relative bg-white rounded-lg border border-slate-200 p-6 w-full max-w-md mx-4 border border-slate-200">
            ...
            {/* Modal footer */}
            <div className="flex items-center justify-end pt-4 mt-4 border-t border-slate-200 gap-3">
              <button
                onClick={() => setShowDeleteModal(false)}
                type="button"
                className="py-2.5 px-5 text-sm font-medium text-slate-700 bg-white border border-slate-300 rounded-lg hover:bg-slate-100 transition-colors"
              >
                Cancel
              </button>
              <button
                ...
                disabled={isDeleting}
                className="px-5 py-2.5 text-sm font-medium text-white bg-red-600 hover:bg-red-700 rounded-lg transition-colors inline-flex items-center disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {isDeleting ? (
                  <>
  ...
  return (
    <nav className="flex gap-1">
      {links.map((link, index) => {
        ...
        if (!link.url) {
          return (
            <span key={index} className="px-3 py-2 text-sm text-gray-400 cursor-not-allowed">
              {label}
            </span>
          );
        }
        ...
        return (
          <Link
            ...
            className={`px-3 py-2 text-sm rounded-lg transition-colors ${
              link.active ? 'bg-primary text-white' : 'text-slate-700 hover:bg-slate-100'
            }`}
            preserveScroll
          >
            {label}

...
export default function Show({ mentorshipRequest, sessions, resources, milestones }: Props) {
  ...
  const getStatusBadge = (status: string) => {
    const badges: Record<string, { bg: string; text: string; label: string }> = {
      ...
      instructor_approved: { bg: 'bg-primary-50', text: 'text-primary-700', label: 'Awaiting Admin Approval' },
      admin_approved: { bg: 'bg-lime-100', text: 'text-lime-700', label: 'Active' },
      instructor_rejected: { bg: 'bg-accent-50', text: 'text-accent', label: 'Instructor Rejected' },
      admin_rejected: { bg: 'bg-accent-50', text: 'text-accent', label: 'Admin Rejected' },
      cancelled: { bg: 'bg-slate-100', text: 'text-slate-700', label: 'Cancelled' },
      ended: { bg: 'bg-slate-100', text: 'text-slate-700', label: 'Ended' },
    };
        ...
        <div className="max-w-7xl mx-auto sm:px-6 lg:px-8">
          ...
          <div className="mb-8">
            ...
            <div className="flex justify-between items-center mt-4">
              <div>
                <h1 className="text-3xl font-bold text-slate-900">Mentorship with {mentorshipRequest.instructor.name}</h1>
              ...
              {activeTab === 'overview' && (
                <div className="space-y-6">
                  ...
                  <div>
                    ...
                    <div className="bg-slate-50 rounded-lg p-4 space-y-3">
                      <div>
                        ...
                        <p className="mt-1 text-sm text-slate-900">{mentorshipRequest.message}</p>
                      </div>
                      {mentorshipRequest.goals && (
                        <div>
                          <label className="block text-sm font-medium text-slate-700">Your Goals</label>
                          <p className="mt-1 text-sm text-slate-900">{mentorshipRequest.goals}</p>
                        </div>
                      )}
                      <div className="grid grid-cols-2 gap-4">
                        <div>
                          <label className="block text-sm font-medium text-slate-700">Duration Type</label>
                          <p className="mt-1 text-sm text-slate-900 capitalize">{mentorshipRequest.duration_type}</p>
                        </div>
                        <div>
                          <label className="block text-sm font-medium text-slate-700">Duration Value</label>
                          <p className="mt-1 text-sm text-slate-900">{mentorshipRequest.duration_value} {mentorshipRequest.duration_type}(s)</p>
                        </div>
                      </div>
                    </div>
                  </div>
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="flex items-start">
                        <div className={`flex-shrink-0 h-10 w-10 rounded-full flex items-center justify-center ${
                          mentorshipRequest.instructor_approved_at ? 'bg-lime-100' : 'bg-yellow-100'
                        }`}>
                          {mentorshipRequest.instructor_approved_at ? (
                            <svg className="h-6 w-6 text-lime-600" fill="currentColor" viewBox="0 0 20 20">
                        ...
                        <div className="ml-4 flex-1">
                          <h4 className="text-sm font-medium text-slate-900">Instructor Review</h4>
                          ...
                          {mentorshipRequest.instructor_response && (
                            <p className="mt-2 text-sm text-slate-700 bg-white p-3 rounded border border-slate-200">
                              {mentorshipRequest.instructor_response}
                            </p>
                          )}
                        </div>
                      ...
                      {mentorshipRequest.instructor_approved_at && (
                        <div className="flex items-start">
                          <div className={`flex-shrink-0 h-10 w-10 rounded-full flex items-center justify-center ${
                            mentorshipRequest.admin_approved_at ? 'bg-lime-100' : 'bg-yellow-100'
                          }`}>
                          ...
                          <div className="ml-4 flex-1">
                            <h4 className="text-sm font-medium text-slate-900">Admin Review</h4>
                            <p className="mt-1 text-sm text-slate-500">
                              {mentorshipRequest.admin_approved_at
                  ...
                  {mentorshipRequest.status === 'admin_approved' && mentorshipRequest.meeting_link && (
                    <div>
                      <h3 className="text-lg font-semibold text-slate-900 mb-4">Meeting Information</h3>
                      <div className="bg-primary-50 rounded-lg p-4 space-y-3">
                        <div>
                          <label className="block text-sm font-medium text-primary-900">Meeting Link</label>
                          <a
                            href={mentorshipRequest.meeting_link}
                            target="_blank"
                            rel="noopener noreferrer"
                            className="mt-1 text-sm text-primary hover:text-primary-800 underline"
                          >
                        ...
                        {mentorshipRequest.next_session_at && (
                          <div>
                            <label className="block text-sm font-medium text-primary-900">Next Session</label>
                            ...
                            <tr key={session.id} className="hover:bg-slate-50">
                              ...
                              <td className="px-6 py-4 whitespace-nowrap">
                                <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${
                                  session.completed_at ? 'bg-lime-100 text-lime-700' : 'bg-yellow-100 text-yellow-800'
                                }`}>
                ...
                <div>
                  ...
                  {resources.length === 0 ? (
                    <div className="text-center py-12 bg-slate-50 rounded-lg">
                      ...
                      <h3 className="mt-2 text-sm font-medium text-slate-900">No resources yet</h3>
                      <p className="mt-1 text-sm text-slate-500">Upload your first learning resource.</p>
                    </div>
                  ) : (
                    <div className="overflow-hidden shadow ring-1 ring-black ring-opacity-5 rounded-lg">
                      <table className="min-w-full divide-y divide-gray-200">
                            ...
                            <tr key={resource.id} className="hover:bg-slate-50">
                              <td className="px-6 py-4 whitespace-nowrap">
                                <div className="flex items-center">
                                  ...
                                  <div>
                                    <div className="text-sm font-medium text-slate-900">{resource.file_name}</div>
                ...
                <div>
                  ...
                  {milestones.length === 0 ? (
                    <div className="text-center py-12 bg-slate-50 rounded-lg">
                      ...
                      <h3 className="mt-2 text-sm font-medium text-slate-900">No milestones yet</h3>
                      <p className="mt-1 text-sm text-slate-500">Create your first learning milestone.</p>
                    </div>
                  ) : (
                    <div className="space-y-4">
                      {milestones.map((milestone) => (
                        <div
                          ...
                          className={`border rounded-lg p-4 ${
                            milestone.completed_at ? 'bg-lime-50 border-lime-200' : 'bg-white border-slate-200'
                          }`}
                        >
                          <div className="flex items-start">
                            ...
                            <div className="ml-3 flex-1">
                              <div className="flex items-center justify-between">
                                <h3 className={`text-lg font-medium ${
                                  milestone.completed_at ? 'text-lime-800 line-through' : 'text-slate-900'
                                }`}>
                                  {milestone.title}
                                </h3>
                                {milestone.due_date && (
                              ...
                              {milestone.description && (
                                <p className={`mt-1 text-sm ${
                                  milestone.completed_at ? 'text-lime-700' : 'text-slate-500'
                                }`}>
                                  {milestone.description}
                                </p>
                              )}
            ...
            <form onSubmit={handleSessionSubmit}>
              <div className="mb-4">
                ...
                <input
                  ...
                  required
                />
                {sessionForm.errors.session_date && (
                  <p className="mt-1 text-sm text-accent">{sessionForm.errors.session_date}</p>
                )}
              </div>
              <div className="mb-4">
                <label className="block text-sm font-medium text-slate-700 mb-2">
            ...
            <form onSubmit={handleResourceSubmit}>
              <div className="mb-4">
                ...
                {resourceForm.errors.file && (
                  <p className="mt-1 text-sm text-accent">{resourceForm.errors.file}</p>
                )}
              </div>
              <div className="mb-4">
                <label className="block text-sm font-medium text-slate-700 mb-2">
            ...
            <form onSubmit={handleMilestoneSubmit}>
              <div className="mb-4">
                ...
                {milestoneForm.errors.title && (
                  <p className="mt-1 text-sm text-accent">{milestoneForm.errors.title}</p>
                )}
              </div>
              <div className="mb-4">
                <label className="block text-sm font-medium text-slate-700 mb-2">
